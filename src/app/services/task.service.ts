...
export class TaskService {
  ...
  // ------------------ Endpoints ------------------ //
    ...
    getAllTask(){
      return this.httpClient.get(this.EndpointApi + `/tasks`, { headers: this.headers });
    }

    getTaskId(id: string){
      return (this.task = this.httpClient.get(this.EndpointApi + `/tasks/${id}`, { headers: this.headers }));
    }

    saveTask(task:InterfaceTask){
      return this.httpClient.post<InterfaceTask>(this.EndpointApi + `/tasks`, task, { headers: this.headers})
      .pipe(map(data => data));
    }

    updateTask(task){
      const taskId = task.taskId;
      return this.httpClient.put<InterfaceTask>(this.EndpointApi + `/tasks/${taskId}`, task, { headers: this.headers})
      .pipe(map(data => data));
    }
}
